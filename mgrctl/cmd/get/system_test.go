...
func TestSystemListPassesSortingToAPI(t *testing.T) {
	client := &api.APIClient{
		...
		Client: &mocks.MockClient{DoFunc: func(req *http.Request) (*http.Response, error) {
			...
			testutils.AssertEquals(t, "wrong sort direction", "true", query.Get("sortDescending"))
	...
	_, _, err := (&systemFetcher{}).List(client, ListOptions{SortBy: "name", SortDescending: true})
